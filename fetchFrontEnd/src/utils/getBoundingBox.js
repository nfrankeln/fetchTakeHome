export default function getBoundingBox(latitude, longitude, distanceInMiles) {
  ...
  const latTop = Math.asin(
    Math.sin(lat1) * Math.cos(angularDistance) +
      Math.cos(lat1) * Math.sin(angularDistance) * Math.cos(0)
  );
  const latBottom = Math.asin(
    Math.sin(lat1) * Math.cos(angularDistance) +
      Math.cos(lat1) * Math.sin(angularDistance) * Math.cos(Math.PI)
  );
  ...
  const lonOffsetRight = Math.asin(
    Math.sin(angularDistance) / Math.cos(latBottom)
  );
